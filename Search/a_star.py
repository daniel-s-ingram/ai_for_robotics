...
def animate(_):
    ...
    if len(open) == 0:
        ...
        if (x, y) == GOAL_NODE:
            ...
            #    bspl = splrep(current_route[:, 1], current_route[:, 0], s=5)
            #    bspl_y = splev(current_route[:, 1], bspl)
            #    route.set_data(current_route[:, 1], bspl_y)
            #else:
            route.set_data(current_route[:, 1], current_route[:, 0])
        return route,
